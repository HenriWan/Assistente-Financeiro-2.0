...
btnGerarResumo.addEventListener("click", async () => {
    ...
    if (!codigo) {
        ...
        resultado.innerHTML = "<p><strong>Digite seu Código de identificação.</strong></p>";
        return;
    }
        ...
        resultado.innerHTML = `
            <p>
                <strong>
                    ...
                    e se os dados foram digitados corretamente.
                </strong>
            </p>
        `;
    }
...
btnEnviarFeedback.addEventListener("click", async () => {
    ...
    if (!mensagem) {
        feedbackStatus.textContent = "Digite uma mensagem antes de enviar.";
        return;
    }
    ...
    try {
        ...
        if (!resposta.ok) {
            feedbackStatus.textContent = dados.erro || "Erro ao enviar feedback.";
            return;
        }

        feedbackStatus.textContent = "Feedback enviado com sucesso. Obrigado!";
        feedbackMensagem.value = "";

    } catch (erro) {
        console.error(erro);
        feedbackStatus.textContent = "Erro ao enviar feedback agora.";
    }
});
